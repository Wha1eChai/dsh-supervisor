...
- Keep the Bundle patch additive. `cordis.patch.yml` inserts repository-owned rows and does not replace unrelated profile configuration. Core Fleet tools may keep their safe read-only deployment default; write modes and optional Consumers belong in the intended Agent composition.
