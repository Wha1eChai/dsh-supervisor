...
export function apply(ctx: Context, config: Config): void {
  ctx.effect(function* () {
    ...
    if (config.controlMode !== 'read-only') {
      yield ctx.tools.register(defineTool({
        name: 'fleet_send',
        description: 'Queue a follow-up for the target\'s next turn. If the target is running, wait for its current turn to end instead of changing that turn. May consume model and tool resources. Receipt means synchronous inbox acceptance only—not claim, turn, or reply completion.',
        parameters: {
      ...
      yield ctx.tools.register(defineTool({
        name: 'fleet_steer',
        description: 'Steer the target at its next step boundary inside the current turn; if idle, wake it to begin a turn with this steering input. Use fleet_send for work that should wait for a running target\'s next turn. May consume model and tool resources. Receipt means synchronous inbox acceptance only—not claim, turn, or reply completion.',
        parameters: {
          selection_handle: { type: 'string', required: true, description: 'Single-attempt selection from fleet_inspect.' },
